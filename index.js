...
module.exports = function (app) {
  ...
  plugin.schema = function () {
    ...
    const okCount = configuredIds.filter((id) => lastReadings.get(id)?.error === undefined).length
    const summary = configuredIds.length === 0
      ? 'No readings yet — start the plugin and wait a few seconds.'
      : `${okCount}/${configuredIds.length} device(s) reporting OK`
  ...
  plugin.start = function (options) {
    ...
    const unknown = devices.filter((d) => !KNOWN_TYPES[d.type])
    if (unknown.length > 0) {
